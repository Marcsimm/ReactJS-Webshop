...
Cart.find({}, function(err, items) {
 ...
 app.post('/addtocart', function(req, res) {
   ...
   var cart = Cart({
    ...
    Cart.find({title : cart.title}, function (err, docs) {
        if (docs.length){
          Cart.findOneAndUpdate({ quantity: docs[0].quantity}, { $inc: {quantity: 1}}, function(err, product) {
      if (err) throw err;
    });
        }else{
 ...
 res.end();
    ...
    _id = req.params._id;
      Cart.findByIdAndUpdate(_id, { quantity: quantity }, function(err, product) {
  if (err) throw err;
  res.end();
});
     });
